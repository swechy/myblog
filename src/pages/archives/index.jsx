...
function Archives(params) {
  ...
  useEffect(() => {
    ...
    let t = searchParams.get('t') || '全篇';
    setIsparam(t);
    if (t !== '全篇') {
      getPostList().then(res => {
        let arr = res.filter(item => item.typename === t);
        setArticles(arr);
        setTotal(arr.length);
      });
    }
  ...
  const [isparam, setIsparam] = useState('全篇');
  const onChange = (page) => {
    console.log(page);
    setCurrent(page);
    window.scrollTo({
        ...
        <div className="page__content__left">
          <div className="el-card cat__title">
            <div onClick={() => navigate("/archives")} className={
              'cat__title__list' +
              (isparam === "全篇" ? ' cat__title__list__active' : '')
            }>
              全篇
            </div>
            {cats.map((item) => (
              <a
                ...
                className={
                  'cat__title__list' +
                  (isparam === item ? ' cat__title__list__active' : '')
                }
              >
                {item}
              </a>
